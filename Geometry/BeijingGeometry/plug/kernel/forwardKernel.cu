...
// z-domian
    ...
    float a1 = fabs(rayVector.x)/rayVectorDomainDim;
    float a2 = fabs(rayVector.y)/rayVectorDomainDim;
...
torch::Tensor forward(torch::Tensor volume, torch::Tensor _volumeSize, torch::Tensor _detectorSize, torch::Tensor projectVector, const long device){
    ...
    for(int batch = 0;batch < volume.size(0); batch++){
        ...
        for (int angle = 0; angle < angles; angle++){
           forwardKernel<<<gridSize, blockSize>>>(outPtrPitch, volumeSize, volumeCenter, detectorSize, detectorCenter, (float*)projectVector[angle].data<float>(), angle);
        }

...
__global__ void backwardKernel(float* volume, const uint3 volumeSize, const uint2 detectorSize, const float* projectVector, const uint index,const int anglesNum,const float3 volumeCenter, const float2 detectorCenter, const uint systemNum){
    // 体素驱动，代表一个体素点
   ...
   for (int k=0;k<volumeSize.z;k++)
   {
        ...
        atomicAdd(&volume[idx], val * 2 * PI / anglesNum / systemNum);
...
torch::Tensor backward(torch::Tensor sino, torch::Tensor _volumeSize, torch::Tensor _detectorSize, torch::Tensor projectVector, const uint systemNum, const long device){
    ...
    for(int batch = 0;batch < sino.size(0); batch++){
        ...
        const dim3 gridSize = dim3(volumeSize.x / BLOCK_X + 1, volumeSize.y / BLOCK_Y + 1, systemNum / BLOCK_Z + 1);
        printf("thread:(%d %d %d)\n",blockSize.x,blockSize.y,blockSize.z);
        printf("block:(%d %d %d)\n",gridSize.x,gridSize.y,gridSize.z);
        for (int angle = 0; angle < angles; angle++){
           backwardKernel<<<gridSize, blockSize>>>(outPtrPitch, volumeSize, detectorSize, (float*)projectVector[angle].data<float>(), angle,angles,volumeCenter,detectorCenter,systemNum);
        }
